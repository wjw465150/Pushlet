 ...
 * to exchange the Pushlet protocol JSON messages.
...
// Get object reference
	...
	_getWebRoot: function() {
		...
		for (var i = 0; i < nodes.length; ++i) {
			...
			if (src) {
				var index = src.indexOf("ajax-pushlet-client-json.js");
				if (index >= 0) {
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4) {
					if (xmlhttp.status == 200) {
						...
						event.put('p_reason', '[pushlet] problem retrieving JSON data:\n' + xmlhttp.statusText);
						PL._onEvent(event);
					}
				}
			};
...
/**  Handle XMLHttpRequest response json. */
	_onResponse: function(json) {
		PL.debug('_onResponse', json);
		var events = PL._rsp2Events(json);
		if (events == null) {
...
/** Convert json response to PushletEvent objects. */
	_rsp2Events: function(json) {
		// check empty response or json document
		if (!json || json.length==0) {
			return null;
		}

		// Convert json doc to array of PushletEvent objects
		var events = new Array(json.length);
		for (i = 0; i < json.length; i++) {
			events[i] = new PushletEvent(json[i]);
		}
